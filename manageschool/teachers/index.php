...
<style>
  ...
  /* Optional: make CBC green, 8-4-4 gray/blue */
  .badge.cbc {
    ...
    function refreshAssignOptions(modalPrefix = '') {
      ajaxRequest({
        ...
        success: function(json) {
          // Subjects
          // Subjects – show CBC / 8-4-4 badge
          const subjectOptions = json.subjects.map(s => {
            let label = s.name;
            if (s.is_cbc === 1 || s.is_cbc === true) {
              label += ' <span class="badge bg-success ms-2">CBC</span>';
            } else {
              label += ' <span class="badge bg-secondary ms-2">8-4-4</span>';
            }
      ...
      if (confirm('Are you sure you want to delete this assignment?')) {
        ajaxRequest({
          data: {
            ...
            teacher_subject_id: $('#editAssignmentId').val()
          },
          success: function(json) {
            alert(json.message);
            $('#editAssignmentModal').modal('hide');
