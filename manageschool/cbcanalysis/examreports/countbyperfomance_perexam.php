<?php
// reports/examreports/countbyperfomance_perexam.php
session_start();
ob_start();
...
$performance_type = isset($_POST['performance_type']) ? $_POST['performance_type'] : 'top';
$student_count = isset($_POST['student_count']) ? (int)$_POST['student_count'] : 10;
...
// Function to get grade from mean points
function getGradeFromMeanPoints($mean_points) {
    if ($mean_points >= 10) return 'A';
    elseif ($mean_points >= 9) return 'A-';
    elseif ($mean_points >= 8) return 'B+';
    elseif ($mean_points >= 7) return 'B';
    elseif ($mean_points >= 6) return 'B-';
    elseif ($mean_points >= 5) return 'C+';
    elseif ($mean_points >= 4) return 'C';
    elseif ($mean_points >= 3) return 'C-';
    elseif ($mean_points >= 2) return 'D+';
    elseif ($mean_points >= 1) return 'D';
    else return 'E';
}
...
// Fetch results and calculate student totals
$results = [];
if ($exam_id) {
    $query = "
        SELECT r.student_id, r.subject_id, r.paper_id, r.score, s.admission_no, s.full_name
        FROM results r
...
// Calculate student performance and subject details
$student_totals = [];
$student_subjects = [];
...
foreach ($subjects as $subject_id => $subject) {
    ...
    if ($subject['use_papers']) {
        foreach ($subject_papers[$subject_id] as $paper) {
            $subject_max += ($paper['max_score'] ?? 100) * ($paper['contribution_percentage'] / 100);
        }
    } else {
        $subject_max = 100;
    }
...
foreach ($results as $result) {
    ...
    if (!isset($student_totals[$student_id])) {
        ...
        $student_subjects[$student_id] = array_fill_keys(array_keys($subjects), ['score' => 0, 'grade' => 'N/A', 'points' => 0]);
    }
    $subject_id = $result['subject_id'];
    $subject_score = 0;
    if ($subjects[$subject_id]['use_papers']) {
        foreach ($subject_papers[$subject_id] as $paper) {
            if ($result['paper_id'] == $paper['paper_id']) {
                $max_score = $paper['max_score'] ?? 100;
                $contribution = $paper['contribution_percentage'] ?? 100;
                if ($max_score > 0) {
                    $subject_score += ($result['score'] / $max_score) * ($contribution / 100) * 100;
                }
            }
        }
    } else {
    ...
    if ($subject_score > 0) {
        ...
        $student_totals[$student_id]['total_marks'] += $subject_score;
        $student_totals[$student_id]['total_points'] += $grade_info['points'];
        $student_totals[$student_id]['subject_count']++;
...
// Calculate average marks, mean points, and grade
$students = [];
foreach ($student_totals as $student_id => $data) {
    $subject_count = $data['subject_count'];
    if ($subject_count > 0) {
        $average_marks = $data['total_marks'] / $subject_count;
        $mean_points = $data['total_points'] / $subject_count;
        $grade = getGradeFromMeanPoints($mean_points);
        $students[] = [
...
// Sort students by total_marks and assign ranks
usort($students, function ($a, $b) use ($performance_type) {
    return $performance_type === 'top' ? ($b['total_marks'] <=> $a['total_marks']) : ($a['total_marks'] <=> $b['total_marks']);
});
...
foreach ($students as $index => $student) {
    if ($index > 0 && $student['total_marks'] == $prev_marks) {
        $student['rank'] = $rank;
        $same_rank_count++;
    } else {
...
$ranked_students = array_slice($ranked_students, 0, $student_count);
?>
...
<style>
  ...
  .performancereport_footer table td {
    padding: 2px 6px;
  }
...
">
  ...
  <div style="flex: 1; text-align: center; font-weight: bold; font-size: 16px;">
    <?php echo ($performance_type === 'top' ? 'Top' : 'Bottom'); ?> <?php echo $student_count; ?> Students: <?php echo htmlspecialchars($exam_name); ?> (<?php echo htmlspecialchars($full_term); ?> - <?php echo $year; ?>)
  </div>
...
<div class="performancereport_container">
  <!-- Header -->
  <div class="performancereport_header">
    <img src="<?php echo htmlspecialchars($school_logo); ?>" alt="Logo" /><br />
    <h2 class="performancereport_h2"><?php echo htmlspecialchars($school['name'] ?? 'KEILA HIGH SCHOOL'); ?></h2>
    <p class="mb-0 fw-bold">PERFORMANCE REPORT</p>
  ...
  <!-- Report Title -->
  <div class="performancereport_title">
    <?php echo ($performance_type === 'top' ? 'TOP' : 'BOTTOM'); ?> <?php echo $student_count; ?> STUDENTS (Examination Analysis)
  </div>

  <!-- Table -->
  <div class="table-responsive">
    <table class="table table-bordered table-sm text-center align-middle mb-2 performancereport_table">
      <thead>
        <tr>
          <th rowspan="2">#</th>
          ...
          <?php foreach ($subjects as $subject_id => $subject): ?>
            <th colspan="1"><?php echo htmlspecialchars($subject['name']); ?> (Grade)</th>
          <?php endforeach; ?>
          <th rowspan="2">Total Marks</th>
          <th rowspan="2">Average Marks</th>
          <th rowspan="2">Grade</th>
          <th rowspan="2">Mean Points</th>
      ...
      <tbody>
        <?php foreach ($ranked_students as $index => $student): ?>
          <tr>
            <td><?php echo $index + 1; ?></td>
            <td><?php echo htmlspecialchars($student['admission_no']); ?></td>
            <td><?php echo htmlspecialchars($student['full_name']); ?></td>
            <?php foreach ($subjects as $subject_id => $subject): ?>
              <td><?php echo $student['subjects'][$subject_id]['grade']; ?></td>
            <?php endforeach; ?>
            <td><?php echo number_format($student['total_marks'], 2); ?></td>
            <td><?php echo number_format($student['average_marks'], 2); ?>%</td>
            <td><?php echo $student['grade']; ?></td>
            <td><?php echo number_format($student['mean_points'], 2); ?></td>
  ...
  <!-- Footer -->
  <footer class="performancereport_footer">
...
function downloadReport() {
    const containers = document.querySelectorAll('.performancereport_container');
    ...
    const opt = {
        margin: 10,
        filename: 'Performance_Report_<?php echo ($performance_type === "top" ? "Top" : "Bottom") . "_" . $student_count . "_" . str_replace(" ", "_", htmlspecialchars($exam_name)) . "_" . $year . ".pdf"; ?>',
