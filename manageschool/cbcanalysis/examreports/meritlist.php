...
$dormitory_id = (int)($_GET['dormitory_id'] ?? 0);
...
// ────────────────────────────────────────────────
// Build student query with JOINs to assignment tables
// ────────────────────────────────────────────────
$students_query = "
    SELECT DISTINCT 
        s.student_id, s.admission_no, s.full_name, st.stream_name,
        d.name AS dormitory_name, h.name AS house_name
    FROM students s
    LEFT JOIN streams st ON s.stream_id = st.stream_id
    LEFT JOIN student_dormitory_assignments da 
        ON da.student_id = s.student_id 
        AND da.is_current = 1
        AND da.academic_year = YEAR(CURDATE())
    LEFT JOIN dormitories d ON da.dormitory_id = d.dormitory_id
    LEFT JOIN student_houses ha 
        ON ha.student_id = s.student_id 
        AND ha.is_current = 1
        AND ha.academic_year = YEAR(CURDATE())
...
if ($stream_id !== 0) {
    ...
    $types .= "i";
    $params[] = $stream_id;
} elseif ($dormitory_id !== 0) {
    $students_query .= " AND da.dormitory_id = ? AND da.is_current = 1";
    $types .= "i";
    $params[] = $dormitory_id;
} elseif ($house_id !== 0) {
    $students_query .= " AND ha.house_id = ? AND ha.is_current = 1";
    $types .= "i";
    $params[] = $house_id;
}
...
// ────────────────────────────────────────────────
// Determine filter text using actual name from first student row
// ────────────────────────────────────────────────
if ($stream_id !== 0) {
    ...
    $filter_text = "Stream: " . ($stream['stream_name'] ?? 'Selected Stream');
    $stmt->close();
} elseif ($dormitory_id !== 0 && !empty($students)) {
    $filter_text = "Dormitory: " . ($students[0]['dormitory_name'] ?? 'Selected Dormitory');
} elseif ($house_id !== 0 && !empty($students)) {
    $filter_text = "House: " . ($students[0]['house_name'] ?? 'Selected House');
}

// ────────────────────────────────────────────────
// Fetch school / class / exam details
// ────────────────────────────────────────────────
...
// ────────────────────────────────────────────────
// Results + aggregates (unchanged logic)
// ────────────────────────────────────────────────
...
<div class="no-print" style="display: flex; align-items: center; justify-content: space-between; background-color: #1a1f71; padding: 10px 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); color: #fff; margin-bottom: 15px; position: sticky; top: 0; z-index: 9999;">
    ...
    <div style="flex: 1; text-align: center; font-weight: bold; font-size: 16px;">
        Merit List: <?php echo htmlspecialchars($class_name . ' - ' . $filter_text . ' - ' . $exam_name . ' (' . $term . ' ' . $year . ')'); ?>
    </div>
...
<div class="meritlist_container">
    ...
    <div class="meritlist_header">
        ...
        <p class="mb-0"><?php echo htmlspecialchars($class_name . ' - ' . $filter_text . ' - ' . $exam_name . ' (' . $term . ' ' . $year . ')'); ?></p>
    </div>
    ...
    <table class="table table-bordered table-sm text-center align-middle mb-2 meritlist_table">
        <thead>
            <tr>
                ...
                <th>Adm No</th>
                <th>Student Name</th>
                <?php if ($stream_id !== 0 || ($stream_id === 0 && $dormitory_id === 0 && $house_id === 0)): ?>
                    <th>Stream</th>
                <?php endif; ?>
                <?php if ($dormitory_id !== 0): ?>
                    <th>Dormitory</th>
                <?php endif; ?>
                <?php if ($house_id !== 0): ?>
                    <th>House</th>
        ...
        <tbody>
            <?php if (empty($aggregates)): ?>
                <tr>
                    <td colspan="<?php
                                    $colspan = 5;
                                    if ($stream_id !== 0 || ($stream_id === 0 && $dormitory_id === 0 && $house_id === 0)) $colspan++;
                                    if ($dormitory_id !== 0) $colspan++;
                                    if ($house_id !== 0) $colspan++;
                                    echo $colspan;
            ...
            <?php else: ?>
                <?php foreach ($aggregates as $index => $agg): ?>
                    <tr>
                        ...
                        <?php endif; ?>
                        <?php if ($dormitory_id !== 0): ?>
                            <td><?php echo htmlspecialchars($agg['dormitory_name']); ?></td>
                        <?php endif; ?>
                        <?php if ($house_id !== 0): ?>
                            <td><?php echo htmlspecialchars($agg['house_name']); ?></td>
                        <?php endif; ?>
...
<script>
    ...
    function downloadReport() {
        ...
        const opt = {
            margin: 10,
            filename: '<?php echo htmlspecialchars($class_name . '_' . str_replace(' ', '_', $filter_text) . '_' . $exam_name . '_' . $term . '_' . $year); ?>.pdf',
            image: {
                type: 'jpeg',
                quality: 0.98
            },
