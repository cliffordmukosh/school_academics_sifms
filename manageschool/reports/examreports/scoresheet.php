...
// ────────────────────────────────────────────────────────────────
// Validation - relaxed for group mode
// ────────────────────────────────────────────────────────────────
if (empty($class_id)) {
  die("Required parameter missing: class_id.<br><pre>Received POST:\n" . print_r($_POST, true) . "</pre>");
}

$is_group_mode = !empty($group_id);

if (!$is_group_mode && empty($stream_id)) {
  die("Required parameter missing: stream_id (non-group mode).<br><pre>Received POST:\n" . print_r($_POST, true) . "</pre>");
}

// ────────────────────────────────────────────────────────────────
// If in group mode and no subject_id was sent → auto-fetch from group
// ────────────────────────────────────────────────────────────────
if ($is_group_mode && empty($subject_id)) {
  $stmt = $conn->prepare("
        SELECT subject_id 
  ...
  if ($result && !empty($result['subject_id'])) {
    ...
    die("No subject found for this custom group (group_id = $group_id).");
  }
}

// Now we should have a valid subject_id
if (empty($subject_id)) {
  die("Failed to determine subject_id.<br><pre>Received POST:\n" . print_r($_POST, true) . "</pre>");
}
...
// ────────────────────────────────────────────────────────────────
// Fetch stream name (only if stream mode)
// ────────────────────────────────────────────────────────────────
$stream_name = '';
if (!$is_group_mode && $stream_id) {
...
// ────────────────────────────────────────────────────────────────
// Fetch group name (only if group mode)
// ────────────────────────────────────────────────────────────────
$group_name = '';
if ($is_group_mode) {
  ...
  $stmt->close();
  $group_name = $group_row['name'] ?? 'Custom Group';
}
...
// ────────────────────────────────────────────────────────────────
// Fetch students (with proper sorting)
// ────────────────────────────────────────────────────────────────
if ($is_group_mode) {
  $stmt = $conn->prepare("
        ...
        ORDER BY 
            CAST(s.admission_no AS UNSIGNED) ASC,   -- numeric part first
            s.admission_no ASC                          -- then alphanumeric sort
    ");
  ...
  $stmt = $conn->prepare("
        ...
        ORDER BY 
            CAST(admission_no AS UNSIGNED) ASC, 
            admission_no ASC
    ");
  $stmt->bind_param("iii", $stream_id, $class_id, $school_id);
}
...
<body>
  ...
  <div class="no-print" style="display:flex;align-items:center;justify-content:space-between;background-color:#1a1f71;padding:10px 20px;border-radius:8px;box-shadow:0 4px 8px rgba(0,0,0,0.2);color:#fff;margin-bottom:15px;position:sticky;top:0;z-index:9999;">
    ...
    <div style="flex:1;text-align:center;font-weight:bold;font-size:16px;">
      Score Sheet: <?php
                    $display = $is_group_mode ? ($group_name ?: 'Custom Group') : ($stream_name ?: '');
                    echo htmlspecialchars(($class['form_name'] ?? '') . ' ' . $display . ' - ' . ($subject['name'] ?? 'Selected Subject'));
                    ?>
    </div>
    <div style="display:flex;gap:8px;align-items:center;">
      <button style="background:#ff6b6b;border:none;padding:6px 12px;border-radius:5px;color:#fff;cursor:pointer;font-size:12px;"
        onmouseover="this.style.backgroundColor='#e55b5b'" onmouseout="this.style.backgroundColor='#ff6b6b'"
        onclick="history.back();">
        <i class="fas fa-arrow-left"></i> Back
      </button>
      <button style="background:#007bff;border:none;padding:6px 12px;border-radius:5px;color:#fff;cursor:pointer;font-size:12px;"
        onmouseover="this.style.backgroundColor='#0069d9'" onmouseout="this.style.backgroundColor='#007bff'"
        onclick="printReport()">
        <i class="fas fa-print"></i> Print
      </button>
      <!-- <button style="background:#20c997;border:none;padding:6px 12px;border-radius:5px;color:#fff;cursor:pointer;font-size:12px;"
      ...
      <tbody>
        <?php foreach ($students as $student): ?>
          <tr>
            ...
            <td class="name-left"><?php echo htmlspecialchars($student['full_name']); ?></td>
            <td></td>
          </tr>
        <?php endforeach; ?>
      </tbody>
  ...
  <script>
    ...
    function downloadReport() {
      ...
      const opt = {
        margin: 10,
        filename: `Score_Sheet_<?php
                                $display = $is_group_mode ? ($group_name ?: 'Custom_Group') : ($stream_name ?: 'Stream');
                                echo htmlspecialchars(($class['form_name'] ?? 'Form') . '_' . $display . '_' . ($subject['name'] ?? 'Subject'));
                                ?>.pdf`,
        image: {
          type: 'jpeg',
          quality: 0.98
