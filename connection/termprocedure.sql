-- sp_generate_term_subject_aggregates
DELIMITER $$
...
BEGIN
    ...
    SELECT 
        ...
        tst.student_id,
        tst.class_id,
        tst.stream_id,  -- Will use the consistent stream_id per student (or NULL if varies)
        tst.term,
        tst.year,
    ...
    WHERE tst.school_id = p_school_id
      AND tst.term = p_term
      AND tst.class_id = p_class_id
      AND tst.year = v_year  -- Ensure year consistency
    ...
    -- Step 4.4: Calculate class and stream positions based on total_points DESC
    ...
    SELECT 
        stra.id,
        stra.total_points,
        ROW_NUMBER() OVER (ORDER BY stra.total_points DESC) AS class_pos
    ...
    SELECT 
        stra.id,
        stra.total_points,
        ROW_NUMBER() OVER (PARTITION BY stra.stream_id ORDER BY stra.total_points DESC) AS stream_pos
    FROM student_term_results_aggregates stra
    WHERE stra.school_id = p_school_id
      AND stra.term = p_term
      AND stra.class_id = p_class_id
