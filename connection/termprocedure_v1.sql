-- sp_generate_term_subject_aggregates (FULL FINAL FIXED VERSION)
DELIMITER $$
...
BEGIN
    ...
    -- Determine the year and min_subjects
    ...
    DELETE FROM term_subject_aggregates WHERE school_id = p_school_id AND term = p_term AND class_id = p_class_id;
    DELETE FROM term_subject_totals WHERE school_id = p_school_id AND term = p_term AND class_id = p_class_id;
    DELETE FROM student_term_results_aggregates WHERE school_id = p_school_id AND term = p_term AND class_id = p_class_id;

    -- Step 1: Subject-level scores (paper-weighted ONLY when use_papers = 1)
    ...
    INSERT INTO tmp_term_subject_scores
    SELECT 
        r.student_id,
        r.subject_id,
        r.exam_id,
    ...
    GROUP BY r.student_id, r.subject_id, r.exam_id, r.class_id, r.stream_id, 
             YEAR(e.created_at), es.use_papers, e.grading_system_id, ts.user_id, e.min_subjects
    ...
    SELECT 
        p_school_id, ts.student_id, ts.subject_id, ts.exam_id, ts.class_id, ts.stream_id, 
        p_term, ts.year, ts.subject_score
    FROM tmp_term_subject_scores ts
    ON DUPLICATE KEY UPDATE subject_score = VALUES(subject_score);

    -- Step 3: Insert per-subject totals & means
    ...
    -- Step 3.1: Update subject grades (consistent with exam procedure)
    ...
    WHERE tst.school_id = p_school_id AND tst.term = p_term AND tst.class_id = p_class_id;

    -- Step 3.2: Update subject teacher remarks (now more specific)
    UPDATE term_subject_totals tst
    SET tst.subject_teacher_remark_text = (
        ...
        WHERE rr.category = 'subject_teacher'
          ...
          AND (rr.class_id = tst.class_id OR rr.class_id IS NULL)
          AND (rr.subject_id = tst.subject_id OR rr.subject_id IS NULL)
          AND rr.grade = tst.subject_grade
        ORDER BY rr.subject_id DESC, rr.class_id DESC, rr.school_id DESC
        LIMIT 1
    )
    WHERE tst.school_id = p_school_id AND tst.term = p_term AND tst.class_id = p_class_id AND tst.subject_grade IS NOT NULL;

    -- Step 3.3: Update subject_points (consistent with exam_aggregates)
    UPDATE term_subject_totals tst
    ...
    -- Step 4: Student-level term aggregates
    ...
    WHERE tst.school_id = p_school_id
      ...
      AND tst.year = v_year
      AND tst.subject_mean > 0
    GROUP BY tst.student_id, tst.class_id, tst.stream_id, tst.term, tst.year, 
             s.kcpe_score, s.kcpe_grade
    ...
    -- Step 4.1: Update overall student grade
    UPDATE student_term_results_aggregates stra
    SET stra.grade = (
        SELECT gr.grade
        FROM grading_rules gr
        WHERE gr.grading_system_id = stra.grading_system_id
          AND FLOOR(stra.total_points) = gr.points
        LIMIT 1
    )
    WHERE stra.school_id = p_school_id AND stra.term = p_term AND stra.class_id = p_class_id AND stra.year = v_year;

    -- Step 4.2 & 4.3: Class teacher and principal remarks
    UPDATE student_term_results_aggregates stra
    SET stra.class_teacher_remark_text = (
        ...
        ORDER BY rr.school_id DESC LIMIT 1
    )
    WHERE stra.school_id = p_school_id AND stra.term = p_term AND stra.class_id = p_class_id AND stra.year = v_year AND stra.grade IS NOT NULL;
    ...
    -- Step 4.4: Stable class & stream positions
    DROP TEMPORARY TABLE IF EXISTS tmp_class_ranking;
    CREATE TEMPORARY TABLE tmp_class_ranking AS
    SELECT stra.id, stra.total_points,
           ROW_NUMBER() OVER (ORDER BY stra.total_points DESC, stra.student_id ASC) AS class_pos,
           COUNT(*) OVER () AS class_total_students
    ...
    SELECT stra.id, stra.total_points,
           ROW_NUMBER() OVER (PARTITION BY stra.stream_id ORDER BY stra.total_points DESC, stra.student_id ASC) AS stream_pos,
           COUNT(*) OVER (PARTITION BY stra.stream_id) AS stream_total_students
    FROM student_term_results_aggregates stra
    WHERE stra.school_id = p_school_id AND stra.term = p_term AND stra.class_id = p_class_id AND stra.year = v_year AND stra.stream_id IS NOT NULL;

    UPDATE student_term_results_aggregates stra
    JOIN tmp_class_ranking tcr ON stra.id = tcr.id
    SET stra.class_position = tcr.class_pos, stra.class_total_students = tcr.class_total_students
    WHERE stra.school_id = p_school_id AND stra.term = p_term AND stra.class_id = p_class_id AND stra.year = v_year;

    UPDATE student_term_results_aggregates stra
    JOIN tmp_stream_ranking tsr ON stra.id = tsr.id
    SET stra.stream_position = tsr.stream_pos, stra.stream_total_students = tsr.stream_total_students
    WHERE stra.school_id = p_school_id AND stra.term = p_term AND stra.class_id = p_class_id AND stra.year = v_year AND stra.stream_id IS NOT NULL;
