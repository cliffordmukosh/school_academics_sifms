...
BEGIN
    -- Step 1: Temporary table for subject-level scores per exam (across all closed/confirmed exams for the school)
    ...
    INSERT INTO tmp_historical_subject_scores (student_id, subject_id, exam_id, class_id, stream_id, term, year, subject_score, grading_system_id, min_subjects)
    SELECT 
        r.student_id,
        r.subject_id,
        r.exam_id,
        ...
        r.stream_id,
        e.term,
        YEAR(e.created_at) AS year,  -- Updated: Extract year from created_at
        CASE 
    ...
    LEFT JOIN teacher_subjects ts
        ...
        AND (ts.academic_year = YEAR(e.created_at) OR ts.academic_year IS NULL)  -- Updated: Use YEAR(e.created_at)
    WHERE e.school_id = p_school_id
      AND e.status = 'closed'
      AND r.status = 'confirmed'
    GROUP BY r.student_id, r.subject_id, r.exam_id, r.class_id, r.stream_id, e.term, YEAR(e.created_at), es.use_papers, e.grading_system_id, e.min_subjects;  -- Updated: Use YEAR(e.created_at)

    -- Step 2: Temporary table for per-subject term totals & means across exams in each term/year
    ...
    INSERT INTO tmp_historical_subject_totals (school_id, student_id, class_id, stream_id, term, year, subject_id, subject_total, subject_mean, exam_count, grading_system_id, min_subjects)
    SELECT 
    ...
    HAVING COUNT(DISTINCT ts.grading_system_id) = 
       AND COUNT(DISTINCT ts.class_id) = 1 
       AND COUNT(DISTINCT ts.stream_id) = 1 
       AND COUNT(DISTINCT ts.min_subjects) = 1
       AND exam_count > 0;

    -- Step 2.1: Update subject grades based on floored subject_mean
    ...
    -- Step 2.2: Update subject_points based on subject_grade
    UPDATE tmp_historical_subject_totals tst
    ...
    -- Step 3: Temporary table for student-level term aggregates
    ...
    INSERT INTO tmp_historical_student_term (school_id, student_id, class_id, stream_id, term, year, total_marks, average, total_points, min_subjects, grading_system_id)
    SELECT 
        tst.school_id,
        tst.student_id,
    ...
    -- Step 4: Insert/Update into historical table
    ...
    ON DUPLICATE KEY UPDATE
        ...
        class_id = VALUES(class_id),  -- Update if class changed historically
        stream_id = VALUES(stream_id);

    -- Step 5: Calculate class and stream positions based on total_points DESC (per term/year/class/stream)
