...
DROP PROCEDURE IF EXISTS sp_generate_exam_aggregates$$
...
BEGIN
    ...
    IF v_min_subjects IS NULL OR v_min_subjects <= 0 THEN
        SET v_min_subjects = 7;
    END IF;

    -- Step 1: Subject-level aggregates — only subjects with actual positive score
        ...
        COALESCE(
            (SELECT gr.grade 
             ...
             WHERE e.exam_id = p_exam_id
               AND ROUND(t.subject_score) BETWEEN gr.min_score AND gr.max_score
        ...
        COALESCE(
            (SELECT rr.remark_text
             ...
             WHERE rr.category = 'subject_teacher'
               AND t.subject_score >= rr.min_score 
               AND t.subject_score <= rr.max_score
               AND (rr.school_id = p_school_id OR rr.school_id IS NULL)
               AND (rr.class_id = t.class_id OR rr.class_id IS NULL)
               AND (rr.stream_id IS NULL)
               AND (rr.subject_id = t.subject_id OR rr.subject_id IS NULL)
    ...
    FROM (
        ...
        GROUP BY r.student_id, r.subject_id, es.use_papers, es.class_id
        HAVING subject_score > 0
    ) t;

    -- Step 2: Insert subject aggregates
    ...
    INSERT INTO tmp_student_aggregates (
        student_id, total_score, mean_score, mean_grade, total_points, 
        remark_text, class_teacher_remark, subject_count
    )
    SELECT
        ts.student_id,
        SUM(ts.subject_score) AS total_score,
        TRUNCATE(SUM(ts.subject_score) / v_min_subjects, 2) AS mean_score,
        COALESCE(
            (SELECT gr.grade
             FROM grading_rules gr
             JOIN exams e ON e.grading_system_id = gr.grading_system_id
             WHERE e.exam_id = p_exam_id
               AND FLOOR(SUM(ts.subject_points) / v_min_subjects) = gr.points
             LIMIT 1),
            'E'
        ) AS mean_grade,
        TRUNCATE(SUM(ts.subject_points) / v_min_subjects, 3) AS total_points,
        COALESCE(
            (SELECT rr.remark_text
             FROM remarks_rules rr
    ...
    -- Step 5: Class positions (stable ranking with tie-breaker)
    UPDATE exam_aggregates ea
    JOIN (
        SELECT ea2.student_id, ea2.class_id,
               RANK() OVER (PARTITION BY ea2.class_id 
                            ORDER BY ea2.total_points DESC, ea2.student_id ASC) AS class_pos
    ...
    -- Step 6: Stream positions (stable ranking with tie-breaker) — FIXED
    UPDATE exam_aggregates ea
    JOIN (
        SELECT ea2.student_id, ea2.stream_id,
               RANK() OVER (PARTITION BY ea2.stream_id 
                            ORDER BY ea2.total_points DESC, ea2.student_id ASC) AS stream_pos
        FROM exam_aggregates ea2
        WHERE ea2.exam_id = p_exam_id AND ea2.school_id = p_school_id
    ) ranked
