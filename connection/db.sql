...
ALTER TABLE subjects ADD COLUMN subject_initial  varchar(10) NULL AFTER type,
ADD COLUMN subject_code INT(11)  NULL  AFTER subject_initial;

update  subjects set subject_initial="BST", subject_code=565 WHERE name="Business Studies";
update  subjects set subject_initial="MAT", subject_code=121 WHERE name="Mathematics";
update  subjects set subject_initial="ENG", subject_code=101 WHERE name="English";
update  subjects set subject_initial="KIS", subject_code=102 WHERE name="Kiswahili";
update  subjects set subject_initial="BIO", subject_code=231 WHERE name="Biology";
update  subjects set subject_initial="PHY", subject_code=232 WHERE name="Physics";
update  subjects set subject_initial="CHE", subject_code=233 WHERE name="Chemistry";
update  subjects set subject_initial="HIS", subject_code=311 WHERE name="History and Government";
update  subjects set subject_initial="GEO", subject_code=312 WHERE name="Geography";
update  subjects set subject_initial="CRE", subject_code=313 WHERE name="C.R.E.";
update  subjects set subject_initial="AGR" subject_code=443 WHERE name="Agriculture";
update  subjects set subject_initial="HSC", subject_code=441 WHERE name="Home Science";
update  subjects set subject_initial="COMP", subject_code=451 WHERE name="Computer Studies";

-- 1. timetable_settings (enhanced with lessons_per_week defaults for compulsory & electives)
DROP TABLE IF EXISTS timetable_settings;  -- Optional: drop if recreating
CREATE TABLE timetable_settings (
    timetable_setting_id INT AUTO_INCREMENT PRIMARY KEY,
    setting_id INT NOT NULL COMMENT 'Link to school_settings.setting_id (term + academic_year)',
    school_id INT NOT NULL,
    
    -- Daily structure
    start_time TIME NOT NULL COMMENT 'First lesson/bell, e.g., 08:00:00',
    end_time TIME NOT NULL COMMENT 'End of last slot, e.g., 16:00:00',
    lesson_duration_minutes INT NOT NULL DEFAULT 40 COMMENT 'KICD standard 40 min per lesson',
    periods_per_day INT NOT NULL DEFAULT 8 COMMENT 'Teaching lessons per day (excluding breaks)',
    total_slots_per_day INT NOT NULL DEFAULT 13 COMMENT 'Total slots incl. breaks (e.g., 13 lessons + 3breaks)',
    
    -- Weekly structure
    days_per_week INT NOT NULL DEFAULT 5 COMMENT 'Mon-Fri usually',
    total_target_lessons_per_week INT NOT NULL DEFAULT 40 COMMENT 'KICD guideline: ~40 teaching lessons/week total',
    
    -- Lessons per week defaults (used during generation)
    compulsory_lessons_per_week INT NOT NULL DEFAULT 6 COMMENT 'Default lessons/week per compulsory subject (e.g., English, Kiswahili, Math, CSL often 6 each)',
    elective_lessons_per_week INT NOT NULL DEFAULT 6 COMMENT 'Default lessons/week per elective subject (usually 6 each for 3 electives = 18)',

    
    -- Break defaults (used when auto-generating slots)
    short_break_duration_minutes INT DEFAULT 10,
    long_break_duration_minutes INT DEFAULT 30,
    lunch_break_duration_minutes INT DEFAULT 60,
    allow_saturday BOOLEAN DEFAULT FALSE,
    ...
    UNIQUE KEY unique_per_term (setting_id)
);

-- 2. time_slots (unchanged – UI selects slot → sets name, minutes, start_time → app calculates end_time & marks is_break)
CREATE TABLE time_slots (
    time_slot_id INT AUTO_INCREMENT PRIMARY KEY,
    timetable_setting_id INT NOT NULL,
    slot_number INT NOT NULL COMMENT '1 to total_slots_per_day',
    start_time TIME NOT NULL,
    end_time TIME NOT NULL COMMENT 'App-calculated: start_time + INTERVAL duration_minutes MINUTE',
    duration_minutes INT NOT NULL,
    is_break BOOLEAN DEFAULT FALSE,
    break_name VARCHAR(100) NULL COMMENT 'Set in UI when marking as break, e.g., "Lunch Break"',
    is_teaching_slot BOOLEAN DEFAULT TRUE COMMENT 'FALSE for breaks',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    FOREIGN KEY (timetable_setting_id) REFERENCES timetable_settings(timetable_setting_id) ON DELETE CASCADE,
    UNIQUE KEY unique_slot (timetable_setting_id, slot_number)
);

-- 3. classrooms (optional, for future)
...
-- 4. timetables (assignments – subject-based for now, teacher/classroom optional/NULL)
CREATE TABLE IF NOT EXISTS timetables (
    ...
    subject_id INT NULL COMMENT 'Subject assigned to this slot (only teaching slots)',
    teacher_id INT NULL COMMENT 'Optional for now',
    classroom_id INT NULL COMMENT 'Optional for now',
    notes TEXT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    UNIQUE KEY no_clash_per_stream_slot_day (stream_id, day_of_week, time_slot_id)
);

-- 5. Permissions (add if not already present)
INSERT IGNORE INTO permissions (name, description, is_global) VALUES
('manage_timetable_settings', 'Configure term timetable settings incl. lessons per week', FALSE),
('manage_time_slots', 'Define periods, mark breaks, set times', FALSE),
('generate_timetables', 'Generate subject-based timetables for streams', FALSE);


ALTER TABLE timetable_settings
    DROP COLUMN IF EXISTS compulsory_lessons_per_week,
    DROP COLUMN IF EXISTS elective_lessons_per_week,
    DROP COLUMN IF EXISTS pe_lessons_per_week,
    DROP COLUMN IF EXISTS csl_lessons_per_week,
    DROP COLUMN IF EXISTS ict_or_ppi_lessons_per_week,
    DROP COLUMN IF EXISTS study_or_other_support_lessons_per_week;

    ALTER TABLE timetable_settings
    ADD COLUMN IF NOT EXISTS compulsory_lessons_per_week INT NOT NULL DEFAULT 6 
        COMMENT 'Weekly lessons for every compulsory subject in this term',

    ADD COLUMN IF NOT EXISTS elective_lessons_per_week   INT NOT NULL DEFAULT 6 
        COMMENT 'Weekly lessons for every elective subject in this term';


ALTER TABLE subjects ADD COLUMN is_cbc bool 

ALTER TABLE grading_rules 
ADD COLUMN is_cbc TINYINT(1) NOT NULL DEFAULT 0 
AFTER description;

-- Optional: generated column for easier display
ALTER TABLE grading_systems
ADD COLUMN IF NOT EXISTS curriculum_label VARCHAR(10) GENERATED ALWAYS AS (
    CASE WHEN is_cbc = 1 THEN 'CBC' ELSE '8-4-4' END
) STORED;



ALTER TABLE classes  ADD COLUMN is_cbc TINYINT(1) NOT NULL DEFAULT 0 

ALTER TABLE remarks_rules
ADD COLUMN is_cbc TINYINT(1) NOT NULL DEFAULT 0 AFTER category;

-- =============================================================================
-- CBC Competency-based Remarks - EXACT wording as requested
-- For Subject Teacher, Class Teacher and Principal
-- =============================================================================

-- 1. Subject Teacher Remarks (subject_teacher)
INSERT INTO remarks_rules (
    school_id,
    min_score,
    max_score,
    grade,
    ...
    (NULL, 90,  100, 'EE1', 'Exceptional',         'subject_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 75,  89,  'EE2', 'Very Good',           'subject_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 58,  74,  'ME1', 'Good',                'subject_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 41,  57,  'ME2', 'Fair',                'subject_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 31,  40,  'AE1', 'Needs Improvement',   'subject_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 21,  30,  'AE2', 'Below Average',       'subject_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 11,  20,  'BE1', 'Well Below Average',  'subject_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 1,   10,  'BE2', 'Minimal',             'subject_teacher', 1, CURRENT_TIMESTAMP);


-- 2. Class Teacher Remarks (class_teacher)
INSERT INTO remarks_rules (
    school_id,
    min_score,
    max_score,
    ...
    (NULL, 90,  100, 'EE1', 'Exceptional',         'class_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 75,  89,  'EE2', 'Very Good',           'class_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 58,  74,  'ME1', 'Good',                'class_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 41,  57,  'ME2', 'Fair',                'class_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 31,  40,  'AE1', 'Needs Improvement',   'class_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 21,  30,  'AE2', 'Below Average',       'class_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 11,  20,  'BE1', 'Well Below Average',  'class_teacher', 1, CURRENT_TIMESTAMP),
    (NULL, 1,   10,  'BE2', 'Minimal',             'class_teacher', 1, CURRENT_TIMESTAMP);


-- 3. Principal Remarks (principal)
INSERT INTO remarks_rules (
    school_id,
    min_score,
    max_score,
    ...
    (NULL, 90,  100, 'EE1', 'Exceptional',         'principal', 1, CURRENT_TIMESTAMP),
    (NULL, 75,  89,  'EE2', 'Very Good',           'principal', 1, CURRENT_TIMESTAMP),
    (NULL, 58,  74,  'ME1', 'Good',                'principal', 1, CURRENT_TIMESTAMP),
    (NULL, 41,  57,  'ME2', 'Fair',                'principal', 1, CURRENT_TIMESTAMP),
    (NULL, 31,  40,  'AE1', 'Needs Improvement',   'principal', 1, CURRENT_TIMESTAMP),
    (NULL, 21,  30,  'AE2', 'Below Average',       'principal', 1, CURRENT_TIMESTAMP),
    (NULL, 11,  20,  'BE1', 'Well Below Average',  'principal', 1, CURRENT_TIMESTAMP),
    (NULL, 1,   10,  'BE2', 'Minimal',             'principal', 1, CURRENT_TIMESTAMP);


    CREATE TABLE cbc_exam_aggregates (
    aggregate_id INT AUTO_INCREMENT PRIMARY KEY,
    school_id INT NOT NULL,
    exam_id INT NOT NULL,
    student_id INT NOT NULL,
    ...
    mean_score DECIMAL(5,2) NOT NULL,
    total_points DECIMAL(7,3) NOT NULL,
    mean_points DECIMAL(5,3) NOT NULL,
    overall_grade VARCHAR(5) NOT NULL,
    class_position INT NULL,
    stream_position INT NULL,
    class_teacher_remark_text VARCHAR(255) NULL,
    principal_remark_text VARCHAR(255) NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_aggregate (school_id, exam_id, student_id),
    FOREIGN KEY (school_id) REFERENCES schools(school_id) ON DELETE CASCADE,
    FOREIGN KEY (exam_id) REFERENCES exams(exam_id) ON DELETE CASCADE,
    FOREIGN KEY (student_id) REFERENCES students(student_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE cbc_exam_subject_aggregates (
    subject_aggregate_id INT AUTO_INCREMENT PRIMARY KEY,
    school_id INT NOT NULL,
    exam_id INT NOT NULL,
    student_id INT NOT NULL,
    ...
    subject_score DECIMAL(7,2) NOT NULL,
    subject_grade VARCHAR(5) NOT NULL,
    subject_points INT NOT NULL,
    subject_teacher_remark_text VARCHAR(255) NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_subject_aggregate (school_id, exam_id, student_id, subject_id),
    FOREIGN KEY (school_id) REFERENCES schools(school_id) ON DELETE CASCADE,
    FOREIGN KEY (exam_id) REFERENCES exams(exam_id) ON DELETE CASCADE,
    FOREIGN KEY (student_id) REFERENCES students(student_id) ON DELETE CASCADE,
    FOREIGN KEY (subject_id) REFERENCES subjects(subject_id) ON DELETE CASCADE
...
-- Term-level subject aggregates (mean across all exams in the term)
CREATE TABLE IF NOT EXISTS cbc_term_subject_aggregates (
    ...
    year YEAR NOT NULL,
    exam_count INT NOT NULL DEFAULT 0,  -- how many exams contributed
    subject_total DECIMAL(10,2) NOT NULL DEFAULT 0.00,   -- sum of subject scores from exams
    subject_mean DECIMAL(7,2) NOT NULL DEFAULT 0.00,     -- subject_total / exam_count
    subject_grade VARCHAR(5) NULL,                       -- e.g. EE1, ME2...
    subject_points DECIMAL(5,3) NULL,                    -- points of subject_mean
    subject_teacher_remark_text VARCHAR(255) NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    UNIQUE KEY unique_term_subject (school_id, student_id, subject_id, term, year),
    FOREIGN KEY (school_id)   REFERENCES schools(school_id) ON DELETE CASCADE,
    FOREIGN KEY (student_id)  REFERENCES students(student_id) ON DELETE CASCADE,
    FOREIGN KEY (class_id)    REFERENCES classes(class_id) ON DELETE CASCADE,
    FOREIGN KEY (stream_id)   REFERENCES streams(stream_id) ON DELETE SET NULL,
...
-- Term-level student overall aggregate
CREATE TABLE IF NOT EXISTS cbc_term_aggregates (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    school_id INT NOT NULL,
    student_id INT NOT NULL,
    class_id INT NOT NULL,
    ...
    term VARCHAR(50) NOT NULL,
    year YEAR NOT NULL,
    subject_count INT NOT NULL DEFAULT 0,           -- number of subjects taken
    total_marks DECIMAL(10,2) NOT NULL DEFAULT 0.00, -- sum of all subject_means
    mean_score DECIMAL(7,2) NULL,                    -- total_marks / subject_count
    total_points DECIMAL(10,3) NOT NULL DEFAULT 0.000, -- sum of subject_points
    mean_points DECIMAL(7,3) NULL,                   -- total_points / subject_count
    ...
    UNIQUE KEY unique_term_student (school_id, student_id, term, year),
    FOREIGN KEY (school_id)   REFERENCES schools(school_id) ON DELETE CASCADE,
    FOREIGN KEY (student_id)  REFERENCES students(student_id) ON DELETE CASCADE,
    FOREIGN KEY (class_id)    REFERENCES classes(class_id) ON DELETE CASCADE,
    FOREIGN KEY (stream_id)   REFERENCES streams(stream_id) ON DELETE SET NULL
...
-- Table for main tickets (headers)
CREATE TABLE support_tickets (
    ticket_id INT AUTO_INCREMENT PRIMARY KEY,
    school_id INT NOT NULL,                    -- From session['school_id']
    user_id INT NOT NULL,                      -- From session['user_id'] (sender)
    subject VARCHAR(255) NOT NULL,             -- e.g., "Issue with exam results"
    status ENUM('open', 'in_progress', 'closed', 'resolved') DEFAULT 'open' NOT NULL,
    priority ENUM('low', 'medium', 'high', 'urgent') DEFAULT 'medium' NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    closed_at TIMESTAMP NULL,                  -- When resolved
    FOREIGN KEY (school_id) REFERENCES schools(school_id) ON DELETE CASCADE,
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    INDEX idx_school_user (school_id, user_id),  -- For quick user ticket lookup
    INDEX idx_status (status)                  -- For admin filtering
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Table for messages/replies in the ticket (conversation)
CREATE TABLE support_messages (
    message_id INT AUTO_INCREMENT PRIMARY KEY,
    ticket_id INT NOT NULL,
    sender_id INT NULL,                        -- user_id if from user; NULL or special ID for admin/email replies
    is_from_admin TINYINT(1) DEFAULT 0,       -- 1 if reply from admin/email, 0 if from user
    message_text TEXT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (ticket_id) REFERENCES support_tickets(ticket_id) ON DELETE CASCADE,
    FOREIGN KEY (sender_id) REFERENCES users(user_id) ON DELETE SET NULL,  -- Allows admin user deletion
    INDEX idx_ticket (ticket_id)               -- For threading
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
